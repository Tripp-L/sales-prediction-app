...
def home():
    ...
    required_files = {
        ...
        'JavaScript': os.path.exists('static/js/main.js'),
        'Template': os.path.exists('templates/index.html')
    }
    
    app.logger.info(f"File check results: {required_files}")
    
    if not all(required_files.values()):
        missing = [k for k, v in required_files.items() if not v]
        app.logger.error(f"Missing required files: {missing}")
        return f"Error: Missing required files: {missing}", 500
...
# Add route to check static files
